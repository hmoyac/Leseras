...
<!-- Beta Skeleton Module for the Schematron 1.5 XML Schema Language.
 ...
 Copyright (c) 2000,2001 Rick Jelliffe and Academia Sinica Computing Center,
 Taiwan
...
<!--
    Version: 2002-06-14
           ...
           * modified the schematron-get-full-path to include the namespace uri
             when there is no prefix.

    Contributors: Rick Jelliffe (original), Oliver Becker (architecture),
             Miloslav Nic (diagnostic, phase, options), 
             Ludwig Svenonius (abstract),
             Uche Ogbuji (misc. bug fixes), 
             Jim Ancona (SAXON workaround),
    ...
    XSLT versions tested and requires small workaround from you
           * Sablotron does not support import, so merge meta-stylesheets by
             hand
           * Xalan for Java 2.0 outputs wrong namespace URI, so alter by hand
             or script
           * Xalan for C 1.0 has problem with key, so edit by hand. Find "KEY"
             below
...
<!-- Note that this namespace is not version specific.
     This program implements schematron 1.5 -->
...
<xsl:template match="sch:schema | schema">
     ...
     <xsl:for-each select="sch:ns | ns">
      ...
      <xsl:if test="not($phase = '#ALL')">
         <xsl:if test="not(sch:phase[@id = $phase] | phase[@id = $phase])">
            <xsl:message>Phase Error: no phase with name <xsl:value-of
         select="$phase"/> has been defined.</xsl:message>
         </xsl:if>
      </xsl:if>
      ...
      mode="schematron-get-full-path"/>
         ...
         <axsl:choose>
            <axsl:when test="not(namespace-uri(.)='') and
         not(contains(name(.), ':'))">
               <axsl:variable name="sq">'</axsl:variable>
               <axsl:value-of select="concat('*:', local-name(.),
            '[namespace-uri(.)=', $sq, namespace-uri(.), $sq, ']')"/>
            </axsl:when>
            <axsl:otherwise>
   ...
   mode="generate-id-from-path">
         <axsl:apply-templates select="parent::*" mode="generate-id-from-path"/>
         <axsl:value-of select="concat('.processing-instruction-',
