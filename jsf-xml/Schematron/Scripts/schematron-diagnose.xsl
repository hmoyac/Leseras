...
<xsl:stylesheet
   ...
   xmlns:util="com.siss.utilxml.LineCount">
...
<xsl:template name="process-report">
	...
	<xsl:param name="diagnostics"/>
	<xsl:text>&#10;</xsl:text>
	<xsl:text>      Condiciones que se cumplen: </xsl:text>
	<xsl:call-template name="process-message">
		<xsl:with-param name="pattern" select="$test"/>
		<xsl:with-param name="role" select="$role"/>
	</xsl:call-template>
...
<xsl:template name="process-message">
   ...
   <xsl:if test="$role">
      ...
      <xsl:text>)</xsl:text>
   </xsl:if>"<xsl:apply-templates  mode="text" />"en
       <axsl:variable name="msj">
       <xsl:apply-templates  mode="text" />
       </axsl:variable>
       <axsl:variable name="nodo">
       <axsl:apply-templates mode="schematron-get-full-path" select="." />
       </axsl:variable>
       <axsl:apply-templates mode="schematron-get-full-path" select="." />
        <axsl:value-of select="util:getLine($nodo,$msj)"/>
