...
public class CumplimientoGeneralBean {
        ...
        try {
            if (validar()) {
                ...
                if (listaReportes != null && listaReportes.size() > 0) {
                    ...
                    sinIndustrias = false;
                    gererarReporteExcelCumplimiento();
                } else {
    ...
    public void gererarReporteExcelCumplimiento() {
        ...
        try {
            if (!getReportes().isEmpty()) {
                ...
                for (CertificadosIceModel modelo : getReportes()) {
                ...
                modeloExcel.setPeriodoInicial(getPeriodoInicial());
                modeloExcel.setPeriodoFinal(UtilListas.arrayToString(getPeriodoFinal().iterator()));
                String strRegion = ExportExcelServlet.getDescripcionRegion(getRegion(), getListaRegiones().iterator());
