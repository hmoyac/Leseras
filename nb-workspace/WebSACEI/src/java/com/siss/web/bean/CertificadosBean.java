...
public class CertificadosBean {
    ...
    public CertificadosBean() {
        ...
        seleccioneRegion = new SelectItem(new Integer(WebConstants.VALOR_INICIAL), regionName.toUpperCase());
        seleccioneComuna = new SelectItem(new Integer(WebConstants.VALOR_INICIAL), comunaName.toUpperCase());
        industriaSinRegistros = new SelectItem(new Integer(WebConstants.VALOR_INICIAL), sinIndustriaName.toUpperCase());
        comunas.clear();
    ...
    public Certificado cargarCertificadoCumplimiento(
        ...
        try {
            ...
            if (certif != null && certif.getParametrosLimite().isEmpty()) {
                certif.getParametrosLimite().add(new CertificadoDetalleLimite(WebConstants.STRING_VACIO, WebConstants.STRING_VACIO, "SIN INCUMPLIMIENTO", WebConstants.STRING_VACIO, WebConstants.STRING_VACIO, WebConstants.STRING_VACIO));
            }
            if (certif != null && certif.getParametrosFrecuencia().isEmpty()) {
                certif.getParametrosFrecuencia().add(new CertificadoDetalleFrecuencia(WebConstants.STRING_VACIO, WebConstants.STRING_VACIO, WebConstants.STRING_VACIO, WebConstants.STRING_VACIO));
            }
        } catch (EntityException ex) {
            String msjDialog = Util.getString(FacesContext.getCurrentInstance(), "ErrorInternoAplicacion") + "<br>" +
                    Util.getString(FacesContext.getCurrentInstance(), "ErrorPeristente");
    ...
    public String cargarNombre(int estadoDescarga, String tipo, int tipoMuestreo, String periodo, int tipoCertificado) {
        String nombre = WebConstants.CERTIFICADO_NOMBRE;
        switch (tipoCertificado) {
            case 0:
                if (estadoDescarga == 0 && tipo.compareToIgnoreCase(WebConstants.CERTIFICADO_DIRECTEMAR) == 0) {
                    nombre = WebConstants.CERTIFICADO_NOMBRE_NO_DESCARGA_DIRECTEMAR;
                }
                else {
                    if (estadoDescarga == 0 && tipo.compareToIgnoreCase(WebConstants.CERTIFICADO_SISS) == 0) {
                        nombre = WebConstants.CERTIFICADO_NOMBRE_NO_DESCARGA_SISS;
                    }
                    else {
                        if (estadoDescarga == 1 && tipoMuestreo == 0) {
                            nombre = WebConstants.CERTIFICADO_NOMBRE_AUTOCONTROL;
                        }
                        else {
                            if (estadoDescarga == 1 && tipoMuestreo == 1) {
                                nombre = WebConstants.CERTIFICADO_NOMBRE_REMUESTREO;
                            }
                            else {
                                nombre = WebConstants.CERTIFICADO_NOMBRE;
                            }
                        }
                    }
                }
                break;
            case 1:
                nombre = WebConstants.CERTIFICADO_NOMBRE_CUMPLIMIENTO;
                break;
            case 2:
                nombre = WebConstants.CERTIFICADO_NOMBRE_CONTROL_DIRECTO;
                break;
            default:
                nombre = WebConstants.CERTIFICADO_NOMBRE;
        }
        nombre = nombre.concat(periodo.concat(".").concat(WebConstants.PDF));
        return nombre;
    }
            ...
            switch (tipoCertificado) {
                case 0: //AUTOCONTROL-REMUESTREO
                    if (estadoDescarga != null && estadoDescarga.intValue() == 0 && tipo != null && tipo.length() > 0) {
                        if (estadoDescarga.intValue() == 0 && tipo.equalsIgnoreCase(WebConstants.CERTIFICADO_DIRECTEMAR)) {
                            xsl = WebConstants.CERTIFICADO_XSL_NO_DESCARGA_DIRECTEMAR;
                        }
                        else {
                            if (estadoDescarga.intValue() == 0 && tipo.equalsIgnoreCase(WebConstants.CERTIFICADO_SISS)) {
                                xsl = WebConstants.CERTIFICADO_XSL_NO_DESCARGA_SISS;
                            }
                            else {
                                xsl = WebConstants.CERTIFICADO_XSL;
                            }
                        }
                    }
                    else {
                    ...
                    break;
                case 1: //CUMPLIMIENTO
                    xsl = WebConstants.CERTIFICADO_XSL_CUMPLIMIENTO;
                    break;
                case 2: //CONTROL DIRECTO
                    xsl = WebConstants.CERTIFICADO_XSL_CONTROL_DIRECTO;
                    break;
